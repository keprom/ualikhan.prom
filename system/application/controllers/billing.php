...
function oplata_import()
   ...
   if ($_POST['ture_id']!=-1) {	$sql_last.=" and ture_id=".$_POST['ture_id']; }
	...
	function nachislenie_v_buhgalteriu()
	{
		...
		set_time_limit(0);
		$db = dbase_open("c:/oplata/nach5.dbf", 2);
		...
		{			
			...
			$db2 = dbase_open("c:/oplata/nach5.dbf", 2);
			foreach ($nach->result() as $n)
			{
				dbase_add_record($db2,
					array (
...
where firm_id={$this->uri->segment(3)} and data_finish is null";
    ...
    function add_periods()
    {
        $months = array(
            ...
            9 => array('name' => 'Сент¤брь', 'end_day' => 30),
            10 => array('name' => 'Окт¤брь', 'end_day' => 31),
            11 => array('name' => 'Но¤брь', 'end_day' => 30),
		...
		public function export_rekvizit_schet()
    {
        ...
        if ($db) {
            ...
            if (!copy("c:/oplata/rschet.dbf", "c:/oplata/ualhn{$month}.dbf")) {
